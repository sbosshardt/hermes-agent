...
| Auto-recall state/Insights telemetry (storage and chat append fix landed; Codex pending) | State/Insights PR only after Codex post-ACK observation and combined review pass. | Additive schema; durable per-session attempt/success/failure/latency, read-only legacy Insights compatibility, best-effort bounded write. Chat observation proves unique freshly composed current turn in final assembled payload after middleware, **not** provider acceptance; Codex requires an accepted turn. No durable append counter. |
